// PYXIS v4.4.0_RADAR (CONTACT INTEL + AIS NAME CACHE)
...
class PyxisMainMenu extends WatchUi.Menu2 {
    function initialize(simMode as Boolean) {
        ...
        // ── SITUATION: most-used tactical views at the very top ──
        addItem(new WatchUi.MenuItem("── SITUATION ──", "", "sep_sit", {}));
        ...
        addItem(new WatchUi.MenuItem("ADSB Track",      "Airspace Monitor",    "adsb",          {}));
        addItem(new WatchUi.MenuItem("ADSB Geo Map",    "Tactical Air Pic",    "adsb_geo",      {}));
        ...
        // ── WEATHER & OCEAN ──
        addItem(new WatchUi.MenuItem("── WEATHER ──",   "", "sep_wx", {}));
        ...
        addItem(new WatchUi.MenuItem("Wave Rose",       "Wave/Swell/Current",  "wind_rose",     {}));
        ...
        addItem(new WatchUi.MenuItem("Interrogate",      "AIS / ADSB Intel",    "interrogate",   {}));
        ...
        addItem(new WatchUi.MenuItem("Topo Map",         "GA 1:25k Terrain",    "topo_map",      {}));
        addItem(new WatchUi.MenuItem("Naut Chart",       "Seamarks + Buoys",    "nautical_map",  {}));
        addItem(new WatchUi.MenuItem("Satellite",         "Sentinel-2 True Color","satellite_map", {}));
        addItem(new WatchUi.MenuItem("Sat + AIS",          "Live Vessels on Sat",  "sat_ais_map",   {}));
        // ── SYSTEMS ──
...
class PyxisMainMenuDelegate extends WatchUi.Menu2InputDelegate {
    ...
    function onSelect(item as WatchUi.MenuItem) as Void {
        ...
        if (id.equals("toggle_sim") || id.equals("onboard_pyxis") || id.equals("pull_nav")) {
            ...
            var wrv = new WindRoseView();
            WatchUi.pushView(wrv, new WindRoseDelegate(wrv), WatchUi.SLIDE_LEFT);
        } else if (id.equals("wind_map")) {
            var wmv = new WindMapView();
            WatchUi.pushView(wmv, new WindMapDelegate(wmv), WatchUi.SLIDE_LEFT);
        } else if (id.equals("adsb")) {
            var av = new AdsbView();
            WatchUi.pushView(av, new AdsbDelegate(av), WatchUi.SLIDE_LEFT);
        } else if (id.equals("adsb_geo")) {
            var agv = new AdsbGeoView();
            WatchUi.pushView(agv, new AdsbGeoDelegate(agv), WatchUi.SLIDE_LEFT);
        } else if (id.equals("topo_map")) {
            var tmv = new TopoMapView(_service);
            WatchUi.pushView(tmv, new TopoMapDelegate(tmv), WatchUi.SLIDE_LEFT);
        } else if (id.equals("nautical_map")) {
            var nmv = new NauticalMapView(_service);
            WatchUi.pushView(nmv, new NauticalMapDelegate(nmv), WatchUi.SLIDE_LEFT);
        } else if (id.equals("satellite_map")) {
            var satv = new SatelliteMapView(_service);
            WatchUi.pushView(satv, new SatelliteMapDelegate(satv), WatchUi.SLIDE_LEFT);
        } else if (id.equals("sat_ais_map")) {
            var saiv = new SatAisMapView(_service);
            WatchUi.pushView(saiv, new SatAisMapDelegate(saiv), WatchUi.SLIDE_LEFT);
        ...
        } else if (id.equals("interrogate")) {
            WatchUi.pushView(new InterrogateMenu(), new InterrogateDelegate(_service), WatchUi.SLIDE_LEFT);
...
class WeatherRadarView extends WatchUi.View {
    ...
    function requestRadarImage() as Void {
        ...
        var lat_s = _service._view._metrics != null && _service._view._metrics.get("lat") != null
            ? _service._view._metrics.get("lat").toString() : "-39.112";
        var lon_s = _service._view._metrics != null && _service._view._metrics.get("lon") != null
            ? _service._view._metrics.get("lon").toString() : "146.471";
        // Encode z/lat/lon into the inner URL so they reach the proxy, not just wsrv.nl
        var innerUrl = "https://benfishmanta.duckdns.org/wx/" + cb + "/weather_map.jpg"
            + "%3Fz%3D" + _zoom.toString()
            + "%26lat%3D" + lat_s
            + "%26lon%3D" + lon_s;
        var url = "https://wsrv.nl/?url=" + innerUrl + "&n=1&output=jpg";
...
// =====================================================================
// TOPO MAP VIEW  (/topo_map  — GA 1:25,000 topographic tiles)
// =====================================================================
class TopoMapView extends WatchUi.View {
    private var _img;
    private var _loading as Boolean = true;
    private var _failed  as Boolean = false;
    private var _zoom    as Number  = 12;  // z=10 ~10nm  z=12 ~2nm  z=14 <1nm
    private var _lastCode as Number = 0;
    private var _service;

    function initialize(service) { View.initialize(); _service = service; }
    function onShow() as Void { requestMap(); }

    function requestMap() as Void {
        _loading = true; _failed = false; WatchUi.requestUpdate();
        var lat_s = _service._view._metrics != null && _service._view._metrics.get("lat") != null
            ? _service._view._metrics.get("lat").toString() : "-38.487";
        var lon_s = _service._view._metrics != null && _service._view._metrics.get("lon") != null
            ? _service._view._metrics.get("lon").toString() : "145.620";
        var cb = System.getTimer().toString();
        var inner = "https://benfishmanta.duckdns.org/topo_map/" + cb + "/topo.jpg"
            + "%3Fz%3D" + _zoom.toString()
            + "%26lat%3D" + lat_s + "%26lon%3D" + lon_s
            + "%26w%3D320%26h%3D320";
        var url = "https://wsrv.nl/?url=" + inner + "&n=1&output=jpg";
        Communications.makeImageRequest(url, {}, { :maxWidth => 454, :maxHeight => 454 }, method(:onReceive));
    }

    function onReceive(code as Number, data as Graphics.BitmapReference or WatchUi.BitmapResource or Null) as Void {
        _loading = false; _lastCode = code;
        if (code == 200 && data != null) { _img = data; } else { _failed = true; }
        WatchUi.requestUpdate();
    }

    function zoomIn()  as Void { if (_zoom < 15) { _zoom++; requestMap(); } }
    function zoomOut() as Void { if (_zoom > 10) { _zoom--; requestMap(); } }

    function onUpdate(dc as Graphics.Dc) as Void {
        var w = dc.getWidth(); var h = dc.getHeight();
        dc.setColor(Graphics.COLOR_BLACK, Graphics.COLOR_BLACK); dc.clear();
        if (_img != null) {
            var bw = _img.getWidth(); var bh = _img.getHeight();
            dc.drawBitmap(w/2 - bw/2, h/2 - bh/2, _img);
        }
        // Zoom scale label
        var labels = {10=>"~10nm", 11=>"~5nm", 12=>"~2nm", 13=>"~1nm", 14=>"0.5nm", 15=>"0.2nm"};
        var scaleStr = labels.get(_zoom);
        if (scaleStr == null) { scaleStr = "z" + _zoom.toString(); }
        dc.setColor(0x00FFAA, Graphics.COLOR_TRANSPARENT);
        dc.drawText(w/2, h - 30, Graphics.FONT_XTINY, "TOPO  " + scaleStr, Graphics.TEXT_JUSTIFY_CENTER);
        if (_loading) {
            dc.setColor(0x00AADD, Graphics.COLOR_TRANSPARENT);
            dc.drawText(w/2, h/2, Graphics.FONT_XTINY, "LOADING TOPO MAP", Graphics.TEXT_JUSTIFY_CENTER);
        } else if (_failed) {
            dc.setColor(Graphics.COLOR_RED, Graphics.COLOR_TRANSPARENT);
            dc.drawText(w/2, h/2, Graphics.FONT_XTINY, "MAP FAILED: " + _lastCode.toString(), Graphics.TEXT_JUSTIFY_CENTER);
        }
        dc.setColor(Graphics.COLOR_RED, Graphics.COLOR_TRANSPARENT);
        dc.drawLine(w/2 - 8, h/2, w/2 + 8, h/2);
        dc.drawLine(w/2, h/2 - 8, w/2, h/2 + 8);
    }
}

class TopoMapDelegate extends WatchUi.BehaviorDelegate {
    private var _view as TopoMapView;
    function initialize(view as TopoMapView) { BehaviorDelegate.initialize(); _view = view; }
    function onBack() as Boolean { WatchUi.popView(WatchUi.SLIDE_IMMEDIATE); return true; }
    function onKey(evt as WatchUi.KeyEvent) as Boolean {
        if (evt.getKey() == WatchUi.KEY_UP)    { _view.zoomOut(); return true; }
        if (evt.getKey() == WatchUi.KEY_DOWN)  { _view.zoomIn();  return true; }
        if (evt.getKey() == WatchUi.KEY_ENTER) { _view.requestMap(); return true; }
        return false;
    }
    function onSwipe(evt as WatchUi.SwipeEvent) as Boolean {
        if (evt.getDirection() == WatchUi.SWIPE_UP   || evt.getDirection() == WatchUi.SWIPE_RIGHT) { _view.zoomIn();  return true; }
        if (evt.getDirection() == WatchUi.SWIPE_DOWN || evt.getDirection() == WatchUi.SWIPE_LEFT)  { _view.zoomOut(); return true; }
        return false;
    }
}

// =====================================================================
// NAUTICAL MAP VIEW  (/nautical_map  — CartoDB Dark + OpenSeaMap overlay)
// =====================================================================
class NauticalMapView extends WatchUi.View {
    private var _img;
    private var _loading as Boolean = true;
    private var _failed  as Boolean = false;
    private var _zoom    as Number  = 12;  // z=10 ~10nm  z=12 ~2nm  z=14 ~0.5nm
    private var _lastCode as Number = 0;
    private var _service;
    ...
    function requestMap() as Void {
        ...
        var lon_s = _service._view._metrics != null && _service._view._metrics.get("lon") != null
            ? _service._view._metrics.get("lon").toString() : "145.620";
        var cb = System.getTimer().toString();
        var inner = "https://benfishmanta.duckdns.org/nautical_map/" + cb + "/chart.jpg"
            + "%3Fz%3D" + _zoom.toString()
            + "%26lat%3D" + lat_s + "%26lon%3D" + lon_s
            + "%26w%3D320%26h%3D320";
        var url = "https://wsrv.nl/?url=" + inner + "&n=1&output=jpg";
    ...
    function zoomIn()  as Void { if (_zoom < 14) { _zoom++; requestMap(); } }
    function zoomOut() as Void { if (_zoom > 10) { _zoom--; requestMap(); } }
    ...
    function onUpdate(dc as Graphics.Dc) as Void {
        ...
        var labels = {10=>"~10nm", 11=>"~5nm", 12=>"~2nm", 13=>"~1nm", 14=>"~0.5nm"};
        var scaleStr = labels.get(_zoom);
        if (scaleStr == null) { scaleStr = "z" + _zoom.toString(); }
        dc.setColor(0x00CCFF, Graphics.COLOR_TRANSPARENT);
        dc.drawText(w/2, h - 30, Graphics.FONT_XTINY, "NAUT  " + scaleStr, Graphics.TEXT_JUSTIFY_CENTER);
        if (_loading) {
            dc.setColor(0x00AADD, Graphics.COLOR_TRANSPARENT);
            dc.drawText(w/2, h/2, Graphics.FONT_XTINY, "LOADING CHART", Graphics.TEXT_JUSTIFY_CENTER);
        } else if (_failed) {
            dc.setColor(Graphics.COLOR_RED, Graphics.COLOR_TRANSPARENT);
            dc.drawText(w/2, h/2, Graphics.FONT_XTINY, "CHART FAILED: " + _lastCode.toString(), Graphics.TEXT_JUSTIFY_CENTER);
        }
        dc.setColor(Graphics.COLOR_RED, Graphics.COLOR_TRANSPARENT);
        dc.drawLine(w/2 - 8, h/2, w/2 + 8, h/2);
        dc.drawLine(w/2, h/2 - 8, w/2, h/2 + 8);
...
class NauticalMapDelegate extends WatchUi.BehaviorDelegate {
    private var _view as NauticalMapView;
    function initialize(view as NauticalMapView) { BehaviorDelegate.initialize(); _view = view; }
    function onBack() as Boolean { WatchUi.popView(WatchUi.SLIDE_IMMEDIATE); return true; }
    function onKey(evt as WatchUi.KeyEvent) as Boolean {
        if (evt.getKey() == WatchUi.KEY_UP)    { _view.zoomOut(); return true; }
        if (evt.getKey() == WatchUi.KEY_DOWN)  { _view.zoomIn();  return true; }
...
// =====================================================================
// SATELLITE MAP VIEW  (/satellite_map  — Sentinel-2 L2A True Colour)
// =====================================================================
class SatelliteMapView extends WatchUi.View {
    private var _img;
    private var _loading as Boolean = true;
    private var _failed  as Boolean = false;
    private var _zoom    as Number  = 11;  // z=10 ~50nm  z=11 ~25nm  z=12 ~12nm  z=13 ~6nm  z=14 ~3nm
    private var _lastCode as Number = 0;
    private var _service;
    ...
    function requestMap() as Void {
        ...
        var lon_s = _service._view._metrics != null && _service._view._metrics.get("lon") != null
            ? _service._view._metrics.get("lon").toString() : "145.620";
        var cb = System.getTimer().toString();
        var inner = "https://benfishmanta.duckdns.org/satellite_map/" + cb + "/sat.jpg"
            + "%3Fz%3D" + _zoom.toString()
            + "%26lat%3D" + lat_s + "%26lon%3D" + lon_s
            + "%26w%3D320%26h%3D320";
        var url = "https://wsrv.nl/?url=" + inner + "&n=1&output=jpg";
    ...
    function onUpdate(dc as Graphics.Dc) as Void {
        ...
        var labels = {10=>"~50nm", 11=>"~25nm", 12=>"~12nm", 13=>"~6nm", 14=>"~3nm"};
        var scaleStr = labels.get(_zoom);
        if (scaleStr == null) { scaleStr = "z" + _zoom.toString(); }
        dc.setColor(0xFF8C42, Graphics.COLOR_TRANSPARENT);
        dc.drawText(w/2, h - 30, Graphics.FONT_XTINY, "SAT  " + scaleStr, Graphics.TEXT_JUSTIFY_CENTER);
        if (_loading) {
            dc.setColor(0xFF8C42, Graphics.COLOR_TRANSPARENT);
            dc.drawText(w/2, h/2, Graphics.FONT_XTINY, "LOADING SATELLITE", Graphics.TEXT_JUSTIFY_CENTER);
        } else if (_failed) {
            dc.setColor(Graphics.COLOR_RED, Graphics.COLOR_TRANSPARENT);
            dc.drawText(w/2, h/2, Graphics.FONT_XTINY, "SAT FAILED: " + _lastCode.toString(), Graphics.TEXT_JUSTIFY_CENTER);
        }
        dc.setColor(Graphics.COLOR_RED, Graphics.COLOR_TRANSPARENT);
        dc.drawLine(w/2 - 8, h/2, w/2 + 8, h/2);
        dc.drawLine(w/2, h/2 - 8, w/2, h/2 + 8);
...
class SatelliteMapDelegate extends WatchUi.BehaviorDelegate {
    private var _view as SatelliteMapView;
    function initialize(view as SatelliteMapView) { BehaviorDelegate.initialize(); _view = view; }
    function onBack() as Boolean { WatchUi.popView(WatchUi.SLIDE_IMMEDIATE); return true; }
    function onKey(evt as WatchUi.KeyEvent) as Boolean {
        if (evt.getKey() == WatchUi.KEY_UP)    { _view.zoomOut(); return true; }
        if (evt.getKey() == WatchUi.KEY_DOWN)  { _view.zoomIn();  return true; }
...
// =====================================================================
// SAT + AIS MAP VIEW  (/sat_ais_map  — Sentinel-2 + live AIS vessels)
// =====================================================================
class SatAisMapView extends WatchUi.View {
    private var _img;
    private var _loading as Boolean = true;
    private var _failed  as Boolean = false;
    private var _zoom    as Number  = 11;
    private var _lastCode as Number = 0;
    public var _service;
    ...
    function requestMap() as Void {
        ...
        var lon_s = _service._view._metrics != null && _service._view._metrics.get("lon") != null
            ? _service._view._metrics.get("lon").toString() : "145.620";
        var cb = System.getTimer().toString();
        var inner = "https://benfishmanta.duckdns.org/sat_ais_map/" + cb + "/sat_ais.jpg"
            + "%3Fz%3D" + _zoom.toString()
            + "%26lat%3D" + lat_s + "%26lon%3D" + lon_s
            + "%26w%3D320%26h%3D320";
        var url = "https://wsrv.nl/?url=" + inner + "&n=1&output=jpg";
    ...
    function zoomIn()  as Void { if (_zoom < 14) { _zoom++; requestMap(); } }
    function zoomOut() as Void { if (_zoom > 9)  { _zoom--; requestMap(); } }
    ...
    function onUpdate(dc as Graphics.Dc) as Void {
        ...
        var labels = {9=>"~100nm", 10=>"~50nm", 11=>"~25nm", 12=>"~12nm", 13=>"~6nm", 14=>"~3nm"};
        var scaleStr = labels.get(_zoom);
        if (scaleStr == null) { scaleStr = "z" + _zoom.toString(); }
        dc.setColor(0x00CCFF, Graphics.COLOR_TRANSPARENT);
        dc.drawText(w/2, h - 30, Graphics.FONT_XTINY, "SAT+AIS  " + scaleStr, Graphics.TEXT_JUSTIFY_CENTER);
        if (_loading) {
            dc.setColor(0x00CCFF, Graphics.COLOR_TRANSPARENT);
            dc.drawText(w/2, h/2, Graphics.FONT_XTINY, "LOADING SAT+AIS", Graphics.TEXT_JUSTIFY_CENTER);
        } else if (_failed) {
            dc.setColor(Graphics.COLOR_RED, Graphics.COLOR_TRANSPARENT);
            dc.drawText(w/2, h/2, Graphics.FONT_XTINY, "FAILED: " + _lastCode.toString(), Graphics.TEXT_JUSTIFY_CENTER);
        }
        dc.setColor(Graphics.COLOR_RED, Graphics.COLOR_TRANSPARENT);
        dc.drawLine(w/2 - 8, h/2, w/2 + 8, h/2);
        dc.drawLine(w/2, h/2 - 8, w/2, h/2 + 8);
...
class SatAisMapDelegate extends WatchUi.BehaviorDelegate {
    private var _view as SatAisMapView;
    function initialize(view as SatAisMapView) { BehaviorDelegate.initialize(); _view = view; }
    function onBack() as Boolean { WatchUi.popView(WatchUi.SLIDE_IMMEDIATE); return true; }
    function onKey(evt as WatchUi.KeyEvent) as Boolean {
        if (evt.getKey() == WatchUi.KEY_UP)    { _view.zoomOut(); return true; }
        if (evt.getKey() == WatchUi.KEY_DOWN)  { _view.zoomIn();  return true; }
        if (evt.getKey() == WatchUi.KEY_ENTER) {
            // Open AIS target list (same flow as Interrogate → AIS CONTACTS)
            _view._service.makeRequest("AIS_LIST_REQ", false, 0.0f);
            return true;
        }
        return false;
    }
...
// =====================================================================
// ADS-B AIRSPACE VIEW  (fetches /adsb_contacts from proxy)
// =====================================================================
class AdsbView extends WatchUi.View {
    private var _contacts as Array = [];
    private var _loading  as Boolean = true;
    private var _failed   as Boolean = false;
    private var _scrollIdx as Number = 0;
    private var _total    as Number = 0;
    private var _cacheAge as Number = 0;
    ...
    function fetch() as Void {
        ...
        _failed  = false;
        WatchUi.requestUpdate();
        // Basic Auth header: admin:manta → YWRtaW46bWFudGE=
        var headers = {
            "Authorization" => "Basic YWRtaW46bWFudGE="
        };
        var options = {
        ...
        Communications.makeWebRequest(
            "https://benfishmanta.duckdns.org/adsb_contacts",
            {},
            options,
            method(:onReceive)
        );
    ...
    function onReceive(code as Number, data as Dictionary or Null) as Void {
        _loading = false;
        if (code == 200 && data != null) {
            var raw = data.get("contacts");
            if (raw != null && raw instanceof Array) {
                _contacts = raw as Array;
            } else {
                _contacts = [];
            }
            var tot = data.get("total");
            if (tot != null) { _total = tot as Number; }
            var age = data.get("cache_age_s");
            if (age != null) { _cacheAge = age as Number; }
            if (_scrollIdx >= _contacts.size()) { _scrollIdx = 0; }
        } else {
            _failed = true;
        }
        WatchUi.requestUpdate();
    ...
    function scroll(dir as Number) as Void {
        ...
        if (_contacts.size() > 0 && _scrollIdx >= _contacts.size()) { _scrollIdx = _contacts.size() - 1; }
        WatchUi.requestUpdate();
    }
    ...
    function onUpdate(dc as Graphics.Dc) as Void {
        ...
        // Header
        dc.setColor(0xAA88FF, Graphics.COLOR_TRANSPARENT); // Violet
        dc.drawText(cx, 6, Graphics.FONT_XTINY, "ADS-B AIRSPACE", Graphics.TEXT_JUSTIFY_CENTER);
        dc.setColor(Graphics.COLOR_DK_GRAY, Graphics.COLOR_TRANSPARENT);
        var ageStr = _cacheAge > 120 ? "STALE" : "LIVE";
        dc.drawText(cx, 22, Graphics.FONT_XTINY, _total.toString() + " ACT " + ageStr, Graphics.TEXT_JUSTIFY_CENTER);
        dc.drawLine(15, 36, w - 15, 36);

        if (_loading) {
            dc.setColor(0x0099FF, Graphics.COLOR_TRANSPARENT);
            dc.drawText(cx, h / 2, Graphics.FONT_SMALL, "LOADING...", Graphics.TEXT_JUSTIFY_CENTER);
            return;
        }
        if (_failed) {
            dc.setColor(Graphics.COLOR_RED, Graphics.COLOR_TRANSPARENT);
            dc.drawText(cx, h / 2 - 10, Graphics.FONT_XTINY, "COMMS FAILED", Graphics.TEXT_JUSTIFY_CENTER);
            dc.drawText(cx, h / 2 + 10, Graphics.FONT_XTINY, "TAP TO RETRY", Graphics.TEXT_JUSTIFY_CENTER);
            return;
        }
        if (_contacts.size() == 0) {
            dc.setColor(Graphics.COLOR_DK_GRAY, Graphics.COLOR_TRANSPARENT);
            dc.drawText(cx, h / 2, Graphics.FONT_XTINY, "NO AIRCRAFT IN RANGE", Graphics.TEXT_JUSTIFY_CENTER);
            return;
        }

        // Compact list — 7 rows at 45px each, first row = selected (highlighted)
        var y = 38;
        var lineH = 45;
        var maxRows = 7;
        var shown = 0;
        var total = _contacts.size();
        for (var i = _scrollIdx; i < total && shown < maxRows; i++) {
            var c = _contacts[i] as Dictionary;
            var pri = c.get("priority");
            var isEmergency = (pri != null && !pri.toString().equals("ROUTINE") && !pri.toString().equals("VFR"));
            var isSelected  = (shown == 0);  // top visible row = actionable

            // Row background
            if (isSelected) {
                dc.setColor(0x0D1A3A, Graphics.COLOR_TRANSPARENT);
                dc.fillRectangle(6, y, w - 12, lineH - 4);
                dc.setColor(0x2244AA, Graphics.COLOR_TRANSPARENT);
                dc.drawRectangle(6, y, w - 12, lineH - 4);
            } else if (isEmergency) {
                dc.setColor(0x2A0000, Graphics.COLOR_TRANSPARENT);
                dc.fillRectangle(8, y, w - 16, lineH - 4);
            }

            // Line 1: callsign | country / squawk
            var call = c.get("callsign");
            if (call == null) { call = c.get("icao"); }
            if (call == null) { call = "?????"; }
            var callStr = call.toString();
            if (callStr.length() > 9) { callStr = callStr.substring(0, 9); }
            dc.setColor(isEmergency ? Graphics.COLOR_RED : (isSelected ? 0x00CCFF : 0xAA88FF), Graphics.COLOR_TRANSPARENT);
            dc.drawText(12, y + 2, Graphics.FONT_XTINY, callStr, Graphics.TEXT_JUSTIFY_LEFT);
            if (isEmergency) {
                var sq = c.get("squawk");
                if (sq != null) {
                    dc.setColor(Graphics.COLOR_ORANGE, Graphics.COLOR_TRANSPARENT);
                    dc.drawText(w - 12, y + 2, Graphics.FONT_XTINY, "SQK " + sq.toString(), Graphics.TEXT_JUSTIFY_RIGHT);
                }
            } else {
                var country = c.get("country");
                if (country != null) {
                    var cStr = country.toString();
                    if (cStr.length() > 10) { cStr = cStr.substring(0, 10); }
                    dc.setColor(Graphics.COLOR_DK_GRAY, Graphics.COLOR_TRANSPARENT);
                    dc.drawText(w - 12, y + 2, Graphics.FONT_XTINY, cStr, Graphics.TEXT_JUSTIFY_RIGHT);
                }
            }

            // Line 2: range+bearing | FL+speed — format floats to avoid "6.600000nm @219.300003°"
            var rng = c.get("range_nm"); var brg = c.get("bearing");
            var rngStr = "?nm";
            if (rng instanceof Float)       { rngStr = (rng as Float).format("%.1f") + "nm"; }
            else if (rng instanceof Number) { rngStr = (rng as Number).toString() + "nm"; }
            else if (rng != null)           { rngStr = rng.toString() + "nm"; }
            var brgStr = "";
            if (brg instanceof Float)       { brgStr = "@" + (brg as Float).format("%.0f") + "\u00b0"; }
            else if (brg instanceof Number) { brgStr = "@" + (brg as Number).toString() + "\u00b0"; }
            else if (brg != null)           { brgStr = "@" + brg.toString() + "\u00b0"; }
            dc.setColor(0x00AAEE, Graphics.COLOR_TRANSPARENT);
            dc.drawText(14, y + 18, Graphics.FONT_XTINY, rngStr + " " + brgStr, Graphics.TEXT_JUSTIFY_LEFT);

            var alt = c.get("alt_ft"); var spd = c.get("spd_kts");
            var altFl = 0;
            if (alt instanceof Number) { altFl = (alt as Number) / 100; }
            else if (alt instanceof Float) { altFl = (alt as Float).toNumber() / 100; }
            var altStr = alt != null ? "FL" + altFl.toString() : "";
            var spdVal = "";
            if (spd instanceof Number)      { spdVal = " " + (spd as Number).toString() + "k"; }
            else if (spd instanceof Float)  { spdVal = " " + (spd as Float).toNumber().toString() + "k"; }
            dc.setColor(Graphics.COLOR_WHITE, Graphics.COLOR_TRANSPARENT);
            dc.drawText(w - 12, y + 18, Graphics.FONT_XTINY, altStr + spdVal, Graphics.TEXT_JUSTIFY_RIGHT);

            // Thin divider
            dc.setColor(0x181828, Graphics.COLOR_TRANSPARENT);
            dc.drawLine(10, y + lineH - 3, w - 10, y + lineH - 3);
        ...
        if (_fetching) {
            ...
            dc.drawText(cx, h - 30, Graphics.FONT_XTINY, "QUERYING INTEL...", Graphics.TEXT_JUSTIFY_CENTER);
        }

        // Footer
        dc.setColor(0x111122, Graphics.COLOR_TRANSPARENT);
        dc.fillRectangle(0, h - 16, w, 16);
        dc.setColor(0x2233AA, Graphics.COLOR_TRANSPARENT);
        dc.drawLine(0, h - 16, w, h - 16);
        var footerStr = (_scrollIdx + 1).toString() + "/" + total.toString() + " ACT  TAP=INTEL";
        dc.setColor(0x4466AA, Graphics.COLOR_TRANSPARENT);
        dc.drawText(cx, h - 14, Graphics.FONT_XTINY, footerStr, Graphics.TEXT_JUSTIFY_CENTER);
    }

    // POST selected contact to /adsb_intel and push AdsbIntelView
    function pullIntelForSelected() as Void {
        if (_contacts.size() == 0 || _scrollIdx >= _contacts.size()) { return; }
        _fetching = true;
        WatchUi.requestUpdate();
        var c = _contacts[_scrollIdx] as Dictionary;
        var body = {
            "icao"     => c.get("icao"),
            ...
            "lon"      => c.get("lon"),
            "alt_ft"   => c.get("alt_ft"),
            "spd_kts"  => c.get("spd_kts"),
            "track"    => c.get("track"),
            "squawk"   => c.get("squawk"),
            "range_nm" => c.get("range_nm"),
        ...
        Communications.makeWebRequest("https://benfishmanta.duckdns.org/adsb_intel",
            body, opts, method(:onIntelResult));
    }

    function onIntelResult(code as Number, data as Dictionary or Null) as Void {
        _fetching = false;
        WatchUi.requestUpdate();
        if (code == 200 && data != null) {
            var lines = data.get("watch_lines");
            if (lines == null) { lines = ["INTEL UNAVAIL"]; }
            var icao = data.get("icao");
            if (icao == null) { icao = "?"; }
            var iv = new AdsbIntelView(icao.toString(), lines as Array);
            WatchUi.pushView(iv, new AdsbIntelDelegate(iv), WatchUi.SLIDE_LEFT);
        }
    }
}

class AdsbDelegate extends WatchUi.BehaviorDelegate {
    private var _view as AdsbView;

    function initialize(view as AdsbView) {
        BehaviorDelegate.initialize();
        _view = view;
    }
    ...
    function onKey(evt as WatchUi.KeyEvent) as Boolean {
        if (evt.getKey() == WatchUi.KEY_DOWN)  { _view.scroll(1);  return true; }
        if (evt.getKey() == WatchUi.KEY_UP)    { _view.scroll(-1); return true; }
        if (evt.getKey() == WatchUi.KEY_ENTER) { _view.pullIntelForSelected(); return true; }
        return false;
    }
    ...
    function onSwipe(evt as WatchUi.SwipeEvent) as Boolean {
        if (evt.getDirection() == WatchUi.SWIPE_UP)   { _view.scroll(1);   return true; }
        if (evt.getDirection() == WatchUi.SWIPE_DOWN) { _view.scroll(-1);  return true; }
        if (evt.getDirection() == WatchUi.SWIPE_LEFT) { _view.fetch();     return true; }  // refresh list
        return false;
    }

    function onTap(evt as WatchUi.ClickEvent) as Boolean {
        _view.pullIntelForSelected();
        return true;
    }
}

// =====================================================================
// ADS-B GEO MAP VIEW — Server-rendered tactical air picture
// Fetches /adsb_radar_map JPEG via wsrv.nl (same as WX Radar)
// =====================================================================
class AdsbGeoView extends WatchUi.View {
    private var _mapImage;
    private var _loading  as Boolean = true;
    private var _failed   as Boolean = false;
    private var _zoom     as Number  = 8;   // z=8 ≈ 50nm (default)
    private var _timer    as Timer.Timer?;
    // Intel-fetch state
    public var _intelState as Number = 0; // 0=idle, 1=fetching contacts, 2=fetching intel
    public var _nearestContact as Dictionary? = null;
    ...
    function onShow() as Void {
        ...
        _timer.start(method(:onRefreshTimer), 30000, true);
    }
    ...
    function onRefreshTimer() as Void { requestMap(); }

    function requestMap() as Void {
        _loading = true; _failed = false;
        WatchUi.requestUpdate();
        var cb  = System.getTimer().toString();
        var url = "https://wsrv.nl/?url=https://benfishmanta.duckdns.org/adsb_radar_map%3Fz%3D"
                  + _zoom.toString() + "%26bust%3D" + cb
                  + "&n=1&output=jpg";
        var options = { :maxWidth => 454, :maxHeight => 454 };
        Communications.makeImageRequest(url, {}, options, method(:onReceive));
    }
    ...
    function onReceive(code as Number, data as Graphics.BitmapReference or WatchUi.BitmapResource or Null) as Void {
        _loading = false;
        if (code == 200 && data != null) { _mapImage = data; } else { _failed = true; }
        WatchUi.requestUpdate();
    }

    function zoomIn()  as Void { if (_zoom < 11) { _zoom++; requestMap(); } }  // z=11 ≈ 5nm (closest)
    function zoomOut() as Void { if (_zoom > 6)  { _zoom--; requestMap(); } }  // z=6  ≈ 200nm (widest)

    // Step 1 of Intel chain: fetch contacts to get nearest
    function fetchNearestIntel() as Void {
        _intelState = 1;
        WatchUi.requestUpdate();
        var headers = { "Authorization" => "Basic YWRtaW46bWFudGE=" };
        var options = {
            :method => Communications.HTTP_REQUEST_METHOD_GET,
            :headers => headers,
            :responseType => Communications.HTTP_RESPONSE_CONTENT_TYPE_JSON
        ...
        Communications.makeWebRequest(
            "https://benfishmanta.duckdns.org/adsb_contacts",
            {}, options, method(:onContactsForIntel)
        );
    }

    // Step 2: got contacts, pick first and fetch intel
    function onContactsForIntel(code as Number, data as Dictionary or Null) as Void {
        if (code == 200 && data != null) {
            var raw = data.get("contacts");
            if (raw != null && raw instanceof Array && (raw as Array).size() > 0) {
                _nearestContact = (raw as Array)[0] as Dictionary;
                _intelState = 2;
                WatchUi.requestUpdate();
                // Build POST body
                var c = _nearestContact as Dictionary;
                var body = {
                    "icao"     => c.get("icao"),
                    "callsign" => c.get("callsign"),
                    "lat"      => c.get("lat"),
                ...
                Communications.makeWebRequest(
                    "https://benfishmanta.duckdns.org/adsb_intel",
                    body, opts, method(:onIntelResult)
                );
                return;
            }
        ...
        _intelState = 0;
        _nearestContact = null;
        WatchUi.requestUpdate();
    }

    // Step 3: intel arrived — push report view
    function onIntelResult(code as Number, data as Dictionary or Null) as Void {
        _intelState = 0;
        if (code == 200 && data != null) {
            var lines = data.get("watch_lines");
    ...
    function onUpdate(dc as Graphics.Dc) as Void {
        ...
        if (_loading) {
            dc.setColor(0x6644CC, Graphics.COLOR_TRANSPARENT);
            dc.drawText(w/2, h/2, Graphics.FONT_XTINY, "LOADING AIR PIC", Graphics.TEXT_JUSTIFY_CENTER);
        } else if (_failed) {
            dc.setColor(Graphics.COLOR_RED, Graphics.COLOR_TRANSPARENT);
            dc.drawText(w/2, h/2, Graphics.FONT_XTINY, "MAP UNAVAIL", Graphics.TEXT_JUSTIFY_CENTER);
        }

        if (_intelState == 1) {
            dc.setColor(0xAA88FF, Graphics.COLOR_TRANSPARENT);
            dc.drawText(w/2, h - 25, Graphics.FONT_XTINY, "FETCHING TGT...", Graphics.TEXT_JUSTIFY_CENTER);
        } else if (_intelState == 2) {
            dc.setColor(0x00CCFF, Graphics.COLOR_TRANSPARENT);
            dc.drawText(w/2, h - 25, Graphics.FONT_XTINY, "QUERYING INTEL...", Graphics.TEXT_JUSTIFY_CENTER);
        } else {
            // Footer: show nm range for current zoom
            var rangeStr = "?nm";
            if      (_zoom == 6)  { rangeStr = "~200nm"; }
            else if (_zoom == 7)  { rangeStr = "~100nm"; }
            else if (_zoom == 8)  { rangeStr = "~50nm";  }
            else if (_zoom == 9)  { rangeStr = "~20nm";  }
            else if (_zoom == 10) { rangeStr = "~10nm";  }
            else if (_zoom == 11) { rangeStr = "~5nm";   }
            dc.setColor(0x4444AA, Graphics.COLOR_TRANSPARENT);
            dc.fillRectangle(0, h - 16, w, 16);
            dc.setColor(0x00CCFF, Graphics.COLOR_TRANSPARENT);
            dc.drawText(w/2, h - 14, Graphics.FONT_XTINY, "AIR  " + rangeStr + "  CROWN=INTEL", Graphics.TEXT_JUSTIFY_CENTER);
        }
    }
}

class AdsbGeoDelegate extends WatchUi.BehaviorDelegate {
    private var _view as AdsbGeoView;

    function initialize(view as AdsbGeoView) {
        BehaviorDelegate.initialize();
        _view = view;
    }
    ...
    function onKey(evt as WatchUi.KeyEvent) as Boolean {
        ...
        if (evt.getKey() == WatchUi.KEY_DOWN)  { _view.zoomIn();   return true; }
        if (evt.getKey() == WatchUi.KEY_ENTER) {
            // Push contact list — user scrolls to desired aircraft then taps for intel
            var av = new AdsbView();
            WatchUi.pushView(av, new AdsbDelegate(av), WatchUi.SLIDE_LEFT);
            return true;
        }
    ...
    function onSwipe(evt as WatchUi.SwipeEvent) as Boolean {
        if (evt.getDirection() == WatchUi.SWIPE_UP)    { _view.zoomIn();   return true; }
        if (evt.getDirection() == WatchUi.SWIPE_DOWN)  { _view.zoomOut();  return true; }
        if (evt.getDirection() == WatchUi.SWIPE_LEFT)  {
            // Open contact list for intel selection
            var av2 = new AdsbView();
            WatchUi.pushView(av2, new AdsbDelegate(av2), WatchUi.SLIDE_LEFT);
            return true;
        }
        return false;
    }
    ...
    function onTap(evt as WatchUi.ClickEvent) as Boolean {
        _view.requestMap(); return true;
    }

    function onNextPage()     as Boolean { _view.zoomIn();  return true; }
    function onPreviousPage() as Boolean { _view.zoomOut(); return true; }
}

// =====================================================================
// ADS-B INTEL VIEW — Gemini-grounded aircraft intelligence report
// =====================================================================
class AdsbIntelView extends WatchUi.View {
    private var _icao    as String;
    private var _lines   as Array;
    private var _scrollY as Number = 0;

    function initialize(icao as String, lines as Array) {
        View.initialize();
        _icao  = icao;
        _lines = lines;
    }

    function scrollUp()   as Void { _scrollY -= 20; if (_scrollY < 0) { _scrollY = 0; } WatchUi.requestUpdate(); }
    function scrollDown() as Void { _scrollY += 20; WatchUi.requestUpdate(); }
    ...
    function onUpdate(dc as Graphics.Dc) as Void {
        ...
        // ── Header bar ───────────────────────────────────────────────
        dc.setColor(0x0D1526, Graphics.COLOR_TRANSPARENT);
        dc.fillRectangle(0, 0, w, 34);
        dc.setColor(0x6644CC, Graphics.COLOR_TRANSPARENT);
        dc.drawLine(0, 34, w, 34);
        dc.setColor(0x00CCFF, Graphics.COLOR_TRANSPARENT);
        dc.drawText(w/2, 3, Graphics.FONT_TINY, "AIR INTEL", Graphics.TEXT_JUSTIFY_CENTER);
        dc.setColor(0xAA88FF, Graphics.COLOR_TRANSPARENT);
        dc.drawText(w/2, 20, Graphics.FONT_XTINY, _icao, Graphics.TEXT_JUSTIFY_CENTER);

        // ── Scrollable content ────────────────────────────────────────
        var y = 40 - _scrollY;
        var BODY_H  = 17;   // body line height
        var SECT_H  = 22;   // section header height

        for (var i = 0; i < _lines.size(); i++) {
            var line = _lines[i];
            if (line == null) { y += BODY_H / 2; continue; }  // blank spacer
            var s = line.toString();

            // Detect section headers
            var isAircraft   = (s.find("AIRCRAFT:")   != null);
            var isOperator   = (s.find("OPERATOR:")   != null);
            var isAssess     = (s.find("ASSESSMENT:") != null);
            var isThreat     = (s.find("THREAT:")     != null);
            var isCpa        = (s.find("CPA:")        != null);
            var isHeader     = isAircraft || isOperator || isAssess || isThreat || isCpa;

            if (isHeader && y > 40) {
                // Thin divider before each section
                if (y >= 36 && y < h - 10) {
                    dc.setColor(0x2A2A4A, Graphics.COLOR_TRANSPARENT);
                    dc.drawLine(16, y - 3, w - 16, y - 3);
                }
            }

            if (y >= 36 && y < h - 18) {
                if (isHeader) {
                    // Section key — colour-coded label in FONT_TINY
                    var hcol = 0x00CCFF;
                    if (isOperator)    { hcol = 0xAA88FF; }
                    else if (isAssess) { hcol = 0x00FF88; }
                    else if (isThreat) {
                        hcol = (s.find("HIGH") != null || s.find("MEDIUM") != null)
                            ? 0xFF3333 : Graphics.COLOR_ORANGE;
                    }
                    else if (isCpa)    { hcol = Graphics.COLOR_YELLOW; }
                    dc.setColor(hcol, Graphics.COLOR_TRANSPARENT);
                    // Section header — centered so circle never clips either side
                    dc.drawText(w/2, y, Graphics.FONT_TINY, s, Graphics.TEXT_JUSTIFY_CENTER);
                } else {
                    // Body text — white, indented, smaller
                    var bcol = Graphics.COLOR_WHITE;
                    if (s.find("HIGH") != null)   { bcol = 0xFF3333; }
                    if (s.find("MEDIUM") != null)  { bcol = Graphics.COLOR_ORANGE; }
                    // Body text — centered for circle safety
                    dc.setColor(bcol, Graphics.COLOR_TRANSPARENT);
                    dc.drawText(w/2, y, Graphics.FONT_XTINY, s, Graphics.TEXT_JUSTIFY_CENTER);
                }
            }
            y += isHeader ? SECT_H : BODY_H;
        }
        ...
        dc.drawLine(0, h - 18, w, h - 18);
        dc.setColor(0x6644CC, Graphics.COLOR_TRANSPARENT);
        dc.drawText(w/2, h - 15, Graphics.FONT_XTINY, "UP / DN  SCROLL", Graphics.TEXT_JUSTIFY_CENTER);
    }
}

class AdsbIntelDelegate extends WatchUi.BehaviorDelegate {
    private var _view as AdsbIntelView;

    function initialize(view as AdsbIntelView) {
        BehaviorDelegate.initialize();
        _view = view;
    }
    ...
    function onSwipe(evt as WatchUi.SwipeEvent) as Boolean {
        if (evt.getDirection() == WatchUi.SWIPE_UP)   { _view.scrollDown(); return true; }
        if (evt.getDirection() == WatchUi.SWIPE_DOWN) { _view.scrollUp();   return true; }
        return false;
    }
}
    ...
    function onReceive(responseCode as Number, data as Dictionary or String or Null) as Void {
        ...
        if (responseCode == 200 && data != null && data instanceof Dictionary) {
            ...
            if (isAisList != null && isAisList instanceof Boolean && isAisList) {
                var contacts = data.get("radar_contacts");
                if (contacts == null) { contacts = []; }
                WatchUi.pushView(new AisListMenu(contacts as Array, null), new AisListDelegate(null, self), WatchUi.SLIDE_UP);
                _view.setStatusText("AIS TGT LIST");
                return;
            }

            var isIntel = data.get("intel_ready");
            if (isIntel != null && isIntel instanceof Boolean && isIntel) {
                var intelData = data.get("intel_data");
                var intelLat  = data.get("lat");
                var intelLon  = data.get("lon");
                if (intelData instanceof Array) {
                    var plat = 0.0d;
                    var plon = 0.0d;
                    if (intelLat instanceof Float)  { plat = (intelLat as Float).toDouble(); }
                    else if (intelLat instanceof Double) { plat = intelLat as Double; }
                    if (intelLon instanceof Float)  { plon = (intelLon as Float).toDouble(); }
                    else if (intelLon instanceof Double) { plon = intelLon as Double; }
                    WatchUi.pushView(
                        new ThreatLogView(intelData as Array, plat, plon),
                        new ThreatLogDelegate(),
                        WatchUi.SLIDE_LEFT
                    );
                    _view.setStatusText("DEEP INTEL");
                    return;
                }
            }
...
class AisListDelegate extends WatchUi.Menu2InputDelegate {
    ...
    private var _service   as GarminBenfishService?;
    function initialize(radarView as OsintRadarView?, service as GarminBenfishService?) {
        Menu2InputDelegate.initialize();
        _radarView = radarView;
        _service   = service;
    }
    function onSelect(item as WatchUi.MenuItem) as Void {
        ...
        var id = item.getId();
        if (id != null && !id.toString().equals("none")) {
            // Push actions sub-menu: MARK TGT / DEEP INTEL / RECALL
            WatchUi.pushView(
                new ContactActionsMenu(id.toString()),
                new ContactActionsDelegate(id.toString(), _radarView, _service),
                WatchUi.SLIDE_DOWN
            );
        } else {
            WatchUi.popView(WatchUi.SLIDE_IMMEDIATE);
...
// ── CONTACT ACTIONS SUB-MENU ─────────────────────────────────────────────────
// Shown after selecting a contact from AIS list. Epix Pro 51mm (454x454px).
// Title: MMSI truncated to 9 chars fits Epix title bar.
// Item labels kept ≤10 chars (FONT_SMALL, 424px effective card width).
class ContactActionsMenu extends WatchUi.Menu2 {
    function initialize(mmsi as String) {
        var title = mmsi.length() > 9 ? mmsi.substring(0, 9) : mmsi;
        Menu2.initialize({:title => title});
        addItem(new WatchUi.MenuItem("MARK TGT",   "Set radar focus",  "mark",   {}));
        addItem(new WatchUi.MenuItem("DEEP INTEL", "Gemini Grounding", "intel",  {}));
        addItem(new WatchUi.MenuItem("RECALL",     "Cached brief",     "recall", {}));
    }
}

class ContactActionsDelegate extends WatchUi.Menu2InputDelegate {
    private var _id      as String;
    private var _radar   as OsintRadarView?;
    private var _service as GarminBenfishService?;
    function initialize(id as String, radar as OsintRadarView?, service as GarminBenfishService?) {
        Menu2InputDelegate.initialize();
        _id = id; _radar = radar; _service = service;
    }
    function onSelect(item as WatchUi.MenuItem) as Void {
        var action = item.getId().toString();
        if (action.equals("mark") && _radar != null) {
            _radar._selectedContactId = _id;
            WatchUi.requestUpdate();
        } else if (action.equals("intel") && _service != null) {
            _service.makeRequest("VESSEL_INTEL:" + _id, false, 0.0f);
        } else if (action.equals("recall") && _service != null) {
            _service.makeRequest("RECALL_INTEL:" + _id, false, 0.0f);
        }
        // Return to radar view: pop ContactActionsMenu then AisListMenu
        WatchUi.popView(WatchUi.SLIDE_IMMEDIATE);
        WatchUi.popView(WatchUi.SLIDE_IMMEDIATE);
    }
    function onBack() as Void {
...
// ── INTERROGATE SUB-MENUS (added below) ──

// Sub-menu: choose AIS or ADSB
class InterrogateMenu extends WatchUi.Menu2 {
    function initialize() {
        Menu2.initialize({:title=>"INTERROGATE"});
        addItem(new WatchUi.MenuItem("AIS CONTACTS",  "Maritime vessels",  "ais",  {}));
        addItem(new WatchUi.MenuItem("ADSB CONTACTS", "Air traffic",       "adsb", {}));
    }
}

class InterrogateDelegate extends WatchUi.Menu2InputDelegate {
    private var _service as GarminBenfishService;
    function initialize(service as GarminBenfishService) {
        Menu2InputDelegate.initialize();
        _service = service;
    ...
    function onSelect(item as WatchUi.MenuItem) as Void {
        ...
        if (id.equals("ais")) {
            // Request AIS list from proxy — comes back via onReceive → ais_list_ready
            _service.makeRequest("AIS_LIST_REQ", false, 0.0f);
            WatchUi.popView(WatchUi.SLIDE_IMMEDIATE);
        } else if (id.equals("adsb")) {
            WatchUi.popView(WatchUi.SLIDE_IMMEDIATE);
    ...
    function onSelect() as Boolean {
        ...
        if (currentViewArray != null && currentViewArray[0] instanceof OsintRadarView) {
            ...
            if (lrView._contacts != null && lrView._contacts.size() > 0) {
                WatchUi.pushView(new AisListMenu(lrView._contacts, lrView), new AisListDelegate(lrView, _service), WatchUi.SLIDE_LEFT);
                return true;
            }
        }
        return false;
...
// =====================================================================
// WAVE / SWELL / CURR / WIND ROSE — Animated Environmental Vector Compass
// Shows: Wave (white), Swell (orange), Current (green), Wind (cyan)
// =====================================================================
class WindRoseView extends WatchUi.View {
    ...
    function getData() as Dictionary { return _data; }
    ...
    function onUpdate(dc as Graphics.Dc) as Void {
        var w = dc.getWidth(); var h = dc.getHeight();
        var cx = w / 2; var cy = h / 2;
        var r  = (w < h ? w : h) / 2 - 32; // keep within bezel
        dc.setColor(Graphics.COLOR_BLACK, Graphics.COLOR_BLACK); dc.clear();
        ...
        if (_loading) {
            dc.setColor(0x00FFAA, Graphics.COLOR_TRANSPARENT);
            dc.drawText(cx, cy, Graphics.FONT_XTINY, "FETCHING...", Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER);
            return;
        }
        if (_failed) {
            dc.setColor(Graphics.COLOR_RED, Graphics.COLOR_TRANSPARENT);
            dc.drawText(cx, cy, Graphics.FONT_XTINY, "DATA ERROR", Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER);
            return;
        }

        // Compass rings (static)
        dc.setColor(0x003300, Graphics.COLOR_TRANSPARENT); dc.setPenWidth(1);
        dc.drawCircle(cx, cy, r);
        ...
        dc.drawText(cx,          cy - r + 10, Graphics.FONT_XTINY, "N", Graphics.TEXT_JUSTIFY_CENTER);
        dc.drawText(cx + r - 12, cy,          Graphics.FONT_XTINY, "E", Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER);
        dc.drawText(cx,          cy + r - 18, Graphics.FONT_XTINY, "S", Graphics.TEXT_JUSTIFY_CENTER);
        dc.drawText(cx - r + 8,  cy,          Graphics.FONT_XTINY, "W", Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER);

        // Value strings (proxy fv helper includes units like 'm' or 'kn')
        var wsp = _data.get("wind_sp");  var wspStr = wsp != null ? wsp.toString() : "?";
        ...
        // Bearing data (Robust lookup: new numeric field -> old string field fallback)
        var wdStr = _data.get("wind_dir");
        var wdR   = _data.get("wind_dir_deg");
        var wd    = (wdR != null) ? wdR.toString().toFloat() : parseBrg(wdStr != null ? wdStr.toString() : null);

        var wvStr = _data.get("wave_dir");
        var wvR   = _data.get("wave_dir_deg");
        var wv    = (wvR != null) ? wvR.toString().toFloat() : parseBrg(wvStr != null ? wvStr.toString() : null);

        var swStr = _data.get("swell_dir");
        var swR   = _data.get("swell_dir_deg");
        var sw    = (swR != null) ? swR.toString().toFloat() : parseBrg(swStr != null ? swStr.toString() : null);

        var cuStr = _data.get("curr_dir");
        var cuR   = _data.get("curr_dir_deg");
        var cd    = (cuR != null) ? cuR.toString().toFloat() : parseBrg(cuStr != null ? cuStr.toString() : null);

        // 1. Wind (Cyan) - Label at tip
        if (wd != null && wd >= 0.0f) {
            var wBlow = (wd.toNumber() + 180) % 360;
            drawVector(dc, cx, cy, r, wBlow.toFloat(), 0x00FFFF, 3, _tick, 110);
            var rad = Math.toRadians(wBlow.toDouble());
            dc.setColor(0x00FFFF, Graphics.COLOR_TRANSPARENT);
            dc.drawText((cx + (r + 13) * Math.sin(rad)).toNumber(), (cy - (r + 13) * Math.cos(rad)).toNumber(),
                        Graphics.FONT_XTINY, "W:" + wspStr, Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER);
        }

        // 2. Wave (White) - Label at 75%
        if (wv != null && wv >= 0.0f) {
            var wvBlow = (wv.toNumber() + 180) % 360;
            drawVector(dc, cx, cy, r - 30, wvBlow.toFloat(), 0xDDDDDD, 2, _tick, 100);
            var rad = Math.toRadians(wvBlow.toDouble());
            dc.setColor(0xDDDDDD, Graphics.COLOR_TRANSPARENT);
            dc.drawText((cx + (r - 10) * Math.sin(rad)).toNumber(), (cy - (r - 10) * Math.cos(rad)).toNumber(),
                        Graphics.FONT_XTINY, "Wv:" + wvhStr, Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER);
        }

        // 3. Swell (Orange) - Label at 45%
        if (sw != null && sw >= 0.0f) {
            var sBlow = (sw.toNumber() + 180) % 360;
            drawVector(dc, cx, cy, r - 60, sBlow.toFloat(), 0xFF8800, 2, _tick, 90);
            var rad = Math.toRadians(sBlow.toDouble());
            dc.setColor(0xFF8800, Graphics.COLOR_TRANSPARENT);
            dc.drawText((cx + (r - 110) * Math.sin(rad)).toNumber(), (cy - (r - 110) * Math.cos(rad)).toNumber(),
                        Graphics.FONT_XTINY, "Sw:" + swhStr, Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER);
        }

        // 4. Current (Green) - Label at 20%
        if (cd != null && cd >= 0.0f) {
            var cBlow = (cd.toNumber() + 180) % 360;
            drawVector(dc, cx, cy, r / 2 - 20, cBlow.toFloat(), 0x00CC00, 2, _tick, 80);
            var rad = Math.toRadians(cBlow.toDouble());
            dc.setColor(0x00CC44, Graphics.COLOR_TRANSPARENT);
            dc.drawText((cx + (r / 2 + 15) * Math.sin(rad)).toNumber(), (cy - (r / 2 + 15) * Math.cos(rad)).toNumber(),
                        Graphics.FONT_XTINY, "C:" + cvStr, Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER);
        }

        // 5. CMEMS Wind (White/Cyan - Wind-Wave)
        var wwd = _data.get("wind_ww_deg");
        var wwdBlow = parseBrg(wwd != null ? wwd.toString() : null);
        if (wwdBlow >= 0.0f) {
            var blow = (wwdBlow.toNumber() + 180) % 360;
            // Draw as a thinner, dashed-look (using lower thick) vector
            drawVector(dc, cx, cy, r - 10, blow.toFloat(), 0xAAAAAA, 1, _tick, 105);
            var rad = Math.toRadians(blow.toDouble());
            dc.setColor(0xAAAAAA, Graphics.COLOR_TRANSPARENT);
            dc.drawText((cx + (r - 2) * Math.sin(rad)).toNumber(), (cy - (r - 2) * Math.cos(rad)).toNumber(),
                        Graphics.FONT_XTINY, "WW", Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER);
        }

        // Bottom strip - simple values
        var row1y = h - 34; var row2y = h - 18;
        dc.setColor(0x00FFFF, Graphics.COLOR_TRANSPARENT); dc.drawText(cx/2,    row1y, Graphics.FONT_XTINY, "W:" + wspStr,  Graphics.TEXT_JUSTIFY_CENTER);
        dc.setColor(0xDDDDDD, Graphics.COLOR_TRANSPARENT); dc.drawText(cx+cx/2, row1y, Graphics.FONT_XTINY, "Wv:" + wvhStr, Graphics.TEXT_JUSTIFY_CENTER);
        dc.setColor(0xFF8800, Graphics.COLOR_TRANSPARENT); dc.drawText(cx/2,    row2y, Graphics.FONT_XTINY, "Sw:" + swhStr, Graphics.TEXT_JUSTIFY_CENTER);
        dc.setColor(0x00CC44, Graphics.COLOR_TRANSPARENT); dc.drawText(cx+cx/2, row2y, Graphics.FONT_XTINY, "C:" + cvStr,  Graphics.TEXT_JUSTIFY_CENTER);
    }
}

// Global drawing helper for animated vectors
function drawVector(dc as Graphics.Dc, cx as Number, cy as Number,
                    radius as Number, brg as Float,
                    color as Number, thick as Number, tick as Number,
                    tailRadius as Number) as Void {
    var rad  = Math.toRadians(brg.toDouble());
    var tipX = (cx + radius * Math.sin(rad)).toNumber();
    var tipY = (cy - radius * Math.cos(rad)).toNumber();
    
    // Start behind origin for a visible tail
    var startX = (cx - tailRadius * Math.sin(rad)).toNumber();
    var startY = (cy + tailRadius * Math.cos(rad)).toNumber();
    ...
    // Fixed Tail Feather (at very end of extended tail)
    dc.setPenWidth(1);
    var tailFX = (startX + 6 * Math.sin(perp)).toNumber();
    var tailFY = (startY - 6 * Math.cos(perp)).toNumber();
    dc.drawLine(startX, startY, tailFX, tailFY);

    // Animated "Walking" Ticks along the shaft
    var spacing = 16;
    var anim_offset = (tick % spacing);
    for (var d = anim_offset; d < radius - 12; d += spacing) {
        if (d < 8) { continue; } // leave center origin clear
        var px = cx + d * Math.sin(rad);
        var py = cy - d * Math.cos(rad);
        var fsize = 6;
        var fx = (px + fsize * Math.sin(perp)).toNumber();
        var fy = (py - fsize * Math.cos(perp)).toNumber();
        dc.drawLine(px.toNumber(), py.toNumber(), fx, fy);
    }
}
...
class WindRoseDelegate extends WatchUi.BehaviorDelegate {
    ...
    function onBack() as Boolean { WatchUi.popView(WatchUi.SLIDE_DOWN); return true; }
    function onSelect() as Boolean {
        var dv = new WaveRoseDataView(_view.getData());
        WatchUi.pushView(dv, new WaveRoseDataDelegate(), WatchUi.SLIDE_LEFT);
        return true;
    }
}

// =====================================================================
// PLAIN LANGUAGE DATA VIEW — Simple list of environmentals
// =====================================================================
class WaveRoseDataView extends WatchUi.View {
    private var _data;
    function initialize(data) { View.initialize(); _data = data; }
    
    function onUpdate(dc as Graphics.Dc) as Void {
        dc.setColor(Graphics.COLOR_BLACK, Graphics.COLOR_BLACK); dc.clear();
        if (_data == null || _data.isEmpty()) {
            dc.setColor(0x00FFAA, -1);
            dc.drawText(dc.getWidth()/2, dc.getHeight()/2, Graphics.FONT_XTINY, "NO TELEMETRY", Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER);
            return;
        }
        var w = dc.getWidth(); var h = dc.getHeight();
        var cx = w / 2;
        ...
        dc.setColor(0x00FFAA, Graphics.COLOR_TRANSPARENT);
        dc.drawText(cx, 25, Graphics.FONT_XTINY, "TACTICAL ENVIRONMENTAL BRIEF", Graphics.TEXT_JUSTIFY_CENTER);
        
        var y = 55;
        var spacing = 26;
        dc.setColor(Graphics.COLOR_WHITE, Graphics.COLOR_TRANSPARENT);
        
        var wsp  = _data.get("wind_sp")   != null ? _data.get("wind_sp").toString()  : "n/a";
        var wspD = _data.get("wind_dir")  != null ? _data.get("wind_dir").toString() : "n/a";
        var wwh  = _data.get("wind_ww_h") != null ? _data.get("wind_ww_h").toString() : "0.0m";
        var wwd  = _data.get("wind_ww_dir")!=null ? _data.get("wind_ww_dir").toString():"n/a";
        var wvh  = _data.get("wave_h")    != null ? _data.get("wave_h").toString()   : "0.0m";
        var wvd  = _data.get("wave_dir")  != null ? _data.get("wave_dir").toString() : "n/a";
        var swh  = _data.get("swell_h")   != null ? _data.get("swell_h").toString()  : "0.0m";
        var swd  = _data.get("swell_dir") != null ? _data.get("swell_dir").toString() : "n/a";
        var cv   = _data.get("curr_v")    != null ? _data.get("curr_v").toString()   : "0.0kn";
        var cd   = _data.get("curr_dir")  != null ? _data.get("curr_dir").toString() : "n/a";
        var sst  = _data.get("sst_c")     != null ? _data.get("sst_c").toString()    : "n/a";
        var warn = _data.get("warnings")  != null ? _data.get("warnings").toString() : "NONE";

        dc.drawText(cx, y, Graphics.FONT_XTINY, "MET WIND: " + wsp + " @ " + wspD, Graphics.TEXT_JUSTIFY_CENTER); y += spacing;
        dc.drawText(cx, y, Graphics.FONT_XTINY, "SEA WIND: " + wwh + " @ " + wwd, Graphics.TEXT_JUSTIFY_CENTER); y += spacing;
        dc.drawText(cx, y, Graphics.FONT_XTINY, "WAVE: " + wvh + " @ " + wvd, Graphics.TEXT_JUSTIFY_CENTER); y += spacing;
        dc.drawText(cx, y, Graphics.FONT_XTINY, "SWELL: " + swh + " @ " + swd, Graphics.TEXT_JUSTIFY_CENTER); y += spacing;
        dc.drawText(cx, y, Graphics.FONT_XTINY, "CURR: " + cv + " @ " + cd, Graphics.TEXT_JUSTIFY_CENTER); y += spacing;
        dc.drawText(cx, y, Graphics.FONT_XTINY, "SEA TEMP: " + sst, Graphics.TEXT_JUSTIFY_CENTER); y += spacing;
        
        // Warnings wrapping (tactical red if not NONE)
        if (!warn.equals("NONE")) {
            dc.setColor(Graphics.COLOR_RED, Graphics.COLOR_TRANSPARENT);
            y += 4;
            var words = splitWords(warn);
            var line = "";
            for (var i = 0; i < words.size(); i++) {
                var w_str = words[i].toString();
                var test = line + w_str + " ";
                if (dc.getTextWidthInPixels(test, Graphics.FONT_XTINY) > w - 60) {
            ...
            dc.drawText(cx, y + 10, Graphics.FONT_XTINY, "NO ACTIVE ALERTS", Graphics.TEXT_JUSTIFY_CENTER);
        }
        
        dc.setColor(0x555555, Graphics.COLOR_TRANSPARENT);
        dc.drawText(cx, h - 25, Graphics.FONT_XTINY, "BACK TO EXIT", Graphics.TEXT_JUSTIFY_CENTER);
    }

    private function splitWords(str) as Array {
        if (str == null) { return [] as Array; }
        var words = [] as Array;
        var start = 0;
        var len = str.length();
        for (var i = 0; i < len; i++) {
        ...
        if (start < len) {
            words.add(str.substring(start, len));
        }
        return words;
    }
}

class WaveRoseDataDelegate extends WatchUi.BehaviorDelegate {
    function initialize() { BehaviorDelegate.initialize(); }
    function onBack() as Boolean { WatchUi.popView(WatchUi.SLIDE_RIGHT); return true; }
}
