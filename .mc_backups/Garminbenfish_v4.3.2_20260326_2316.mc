// PYXIS v4.3.2_RADAR (WIND ROSE & MAP)
...
class WindRoseView extends WatchUi.View {
    ...
    function onUpdate(dc as Graphics.Dc) as Void {
        ...
        dc.setColor(0x224422, -1);
        dc.drawLine(cx, cy - r, cx, cy + r);
        dc.drawLine(cx - r, cy, cx + r, cy);

        // NSEW labels (inside ring to avoid edge clipping)
        dc.setColor(0x446644, -1);
        dc.drawText(cx,          cy - r + 10, Graphics.FONT_XTINY, "N", Graphics.TEXT_JUSTIFY_CENTER);
        dc.drawText(cx + r - 12, cy,          Graphics.FONT_XTINY, "E", Graphics.TEXT_JUSTIFY_CENTER);
        dc.drawText(cx,          cy + r - 18, Graphics.FONT_XTINY, "S", Graphics.TEXT_JUSTIFY_CENTER);
        dc.drawText(cx - r + 7,  cy,          Graphics.FONT_XTINY, "W", Graphics.TEXT_JUSTIFY_CENTER);

        // Pre-fetch values for tip labels + bottom strip
        ...
        // Wind arrow — CYAN, pulses; tip = where blowing TO, tail label = FROM direction
        var wd = parseBrg(_data.get("wind_dir") as String);
        if (wd >= 0.0f) {
            var wBlow = (wd.toNumber() + 180) % 360;
            var wR = (_tick % 2 == 0) ? r - 12 : r - 15;
            drawArrow(dc, cx, cy, wR, wBlow.toFloat(), 0x00FFFF, 3);
            // Tip label (speed) at arrowhead
            ...
            // FROM label at tail (28px toward origin bearing)
            var wFromRad = Math.toRadians(wd.toDouble());
            dc.setColor(0x009999, -1);
            dc.drawText((cx + 28 * Math.sin(wFromRad)).toNumber(), (cy - 28 * Math.cos(wFromRad)).toNumber(),
                        Graphics.FONT_XTINY, abbr(_data.get("wind_dir") as String), Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER);
        }
        // Wave arrow — WHITE; tip = travel direction, tail = FROM
        var wvd = parseBrg(_data.get("wave_dir") as String);
        if (wvd >= 0.0f) {
            var wvBlow = (wvd.toNumber() + 180) % 360;
            ...
            var wvFromRad = Math.toRadians(wvd.toDouble());
            dc.setColor(0x888888, -1);
            dc.drawText((cx + 28 * Math.sin(wvFromRad)).toNumber(), (cy - 28 * Math.cos(wvFromRad)).toNumber(),
                        Graphics.FONT_XTINY, abbr(_data.get("wave_dir") as String), Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER);
        }
        // Swell arrow — ORANGE; tip = travel direction, tail = FROM
        var sd = parseBrg(_data.get("swell_dir") as String);
        if (sd >= 0.0f) {
            var sBlow = (sd.toNumber() + 180) % 360;
            ...
            var sFromRad = Math.toRadians(sd.toDouble());
            dc.setColor(0x884400, -1);
            dc.drawText((cx + 28 * Math.sin(sFromRad)).toNumber(), (cy - 28 * Math.cos(sFromRad)).toNumber(),
                        Graphics.FONT_XTINY, abbr(_data.get("swell_dir") as String), Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER);
        }
        // Current arrow — GREEN, inner ring; tip = flow direction, tail = FROM
        var cd = parseBrg(_data.get("curr_dir") as String);
        if (cd >= 0.0f) {
            ...
            var cFromRad = Math.toRadians(cd.toDouble());
            dc.setColor(0x006600, -1);
            dc.drawText((cx + 28 * Math.sin(cFromRad)).toNumber(), (cy - 28 * Math.cos(cFromRad)).toNumber(),
                        Graphics.FONT_XTINY, abbr(_data.get("curr_dir") as String), Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER);
        }
