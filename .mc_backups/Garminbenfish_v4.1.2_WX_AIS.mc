// PYXIS v4.3.1_RADAR (LIVE RADAR HOTFIX - RESTORED)
...
class PyxisMainMenu extends WatchUi.Menu2 {
    function initialize(simMode as Boolean) {
        Menu2.initialize({:title=>"PYXIS 2nd MATE"});
        var modeStr = simMode ? "Data: MANTA_SIM" : "Data: LIVE_AIS";
        addItem(new WatchUi.MenuItem(modeStr, "Toggle Source", "toggle_sim", {}));
        addItem(new WatchUi.MenuItem("Onboard Pyxis", "Watch GPS Only", "onboard_pyxis", {}));
        addItem(new WatchUi.MenuItem("Nav Importer", "Markers & Fixes", "pull_nav", {}));
        ...
        addItem(new WatchUi.MenuItem("Morning Briefing", "Full Day Prep", "day_brief", {}));
        addItem(new WatchUi.MenuItem("Evening Briefing", "Full Night Prep", "night_brief", {}));
        addItem(new WatchUi.MenuItem("Active Route", "Plot 30pt Vector", "item_nav", {}));
        addItem(new WatchUi.MenuItem("3D Sonar", "Sea Floor Matrix", "sonar", {}));
        addItem(new WatchUi.MenuItem("Live Radar", "Local AIS Feed", "map", {}));
        ...
        addItem(new WatchUi.MenuItem("AI Voice", "Query ElevenLabs", "voice", {}));
        addItem(new WatchUi.MenuItem("Environment", "Swells & Temp", "sensor", {}));
        addItem(new WatchUi.MenuItem("Engine Room", "Gen/Bilge Control", "engine", {}));
    }
}
...
class GarminBenfishService {
            ...
            if (isMap != null && isMap instanceof Boolean && isMap) {
                ...
                if (lat != null && lon != null && ((lat instanceof Float || lat instanceof Double) && (lon instanceof Float || lon instanceof Double))) {
                    ...
                    if (isOsint) {
                        ...
                        if (currentView != null && currentView[0] instanceof OsintRadarView) {
                            var orView = currentView[0] as OsintRadarView;
                            orView._contacts = contacts as Array;
                            WatchUi.requestUpdate();
                        } else {
                            var newView = new OsintRadarView(contacts as Array, l_d, lo_d, self);
                            WatchUi.pushView(newView, new OsintRadarDelegate(self), WatchUi.SLIDE_UP);
                        }
    ...
    private var _pyxisLat as Double = 0.0d;
    private var _pyxisLon as Double = 0.0d;
...
class OsintRadarView extends WatchUi.View {
    ...
    public var _radiusMiles as Double = 500.0d; // Strategic scale
    ...
    function zoomIn() as Void {
        ...
        if (_radiusMiles < 25.0d) { _radiusMiles = 25.0d; }
        WatchUi.requestUpdate();
    }
    ...
    function onUpdate(dc as Graphics.Dc) as Void {
        ...
        // Draw Pyxis (White Triangle)
        dc.setColor(Graphics.COLOR_WHITE, Graphics.COLOR_TRANSPARENT);
        dc.fillPolygon([[cx, cy - 6], [cx - 4, cy + 4], [cx + 4, cy + 4]] as Array);
        ...
        for (var i = 0; i < _contacts.size(); i++) {
            ...
            if (c instanceof Dictionary) {
                var brg = c.get("bearing");
                var dist = c.get("range_nm");
                if (brg != null && dist != null) {
                    var brg_d = 0.0d; 
                    if (brg instanceof Float) { brg_d = brg.toDouble(); } 
                    else if (brg instanceof Double) { brg_d = brg; } 
                    else if (brg instanceof Number) { brg_d = brg.toFloat().toDouble(); }

                    var dist_d = 0.0d; 
                    if (dist instanceof Float) { dist_d = dist.toDouble(); } 
                    else if (dist instanceof Double) { dist_d = dist; } 
                    else if (dist instanceof Number) { dist_d = dist.toFloat().toDouble(); }

                    if (dist_d <= _radiusMiles) {
                        var relBrg = brg_d - pyxisHeading; 
                        var theta = Math.toRadians(relBrg - 90);
                        var pixelDist = (dist_d / _radiusMiles) * r;
                        var tx = cx + (Math.cos(theta) * pixelDist);
                        var ty = cy + (Math.sin(theta) * pixelDist);

                        var type = c.get("type");
                        var cid = c.get("id");

                        // OSINT Custom Symbology
                        if (type != null && type.equals("OSINT_MILITARY")) {
                            dc.setColor(Graphics.COLOR_RED, Graphics.COLOR_TRANSPARENT); // Red Star representation (layered triangles)
                            dc.fillPolygon([[tx, ty - 6], [tx - 4, ty + 4], [tx + 4, ty + 4]] as Array);
                            dc.fillPolygon([[tx, ty + 6], [tx - 4, ty - 4], [tx + 4, ty - 4]] as Array);
                        } else if (type != null && type.equals("OSINT_WEATHER")) {
                            dc.setColor(Graphics.COLOR_BLUE, Graphics.COLOR_TRANSPARENT); // Blue Hexagon/Diamond
                            dc.fillPolygon([[tx, ty - 6], [tx + 6, ty], [tx, ty + 6], [tx - 6, ty]] as Array);
                        } else if (type != null && type.equals("OSINT_PIRACY")) {
                            dc.setColor(Graphics.COLOR_YELLOW, Graphics.COLOR_TRANSPARENT); // Yellow Square
                            dc.fillRectangle(tx - 4, ty - 4, 8, 8);
                        } else if (type != null && type.equals("MARKER")) {
                            dc.setColor(Graphics.COLOR_YELLOW, Graphics.COLOR_TRANSPARENT);
                            dc.drawCircle(tx, ty, 3);
                            dc.drawLine(tx, ty - 4, tx, ty + 4);
                            dc.drawLine(tx - 4, ty, tx + 4, ty);
                        } else if (type != null && type.equals("SHOAL")) {
                            dc.setColor(Graphics.COLOR_ORANGE, Graphics.COLOR_TRANSPARENT);
                            dc.fillRectangle(tx - 3, ty - 3, 6, 6);
                            dc.setColor(Graphics.COLOR_BLACK, Graphics.COLOR_TRANSPARENT);
                            dc.fillRectangle(tx - 1, ty - 1, 2, 2);
                        } else if (type != null && (type.equals("SHIP") || type.equals("VESSEL"))) {
                            dc.setColor(Graphics.COLOR_GREEN, Graphics.COLOR_TRANSPARENT);
                            dc.fillPolygon([[tx, ty - 5], [tx - 3, ty + 4], [tx + 3, ty + 4]] as Array);
                        } else {
                            dc.setColor(Graphics.COLOR_GREEN, Graphics.COLOR_TRANSPARENT);
                            dc.fillPolygon([[tx, ty - 5], [tx - 3, ty + 4], [tx + 3, ty + 4]] as Array);
                        }
                        ...
                        // Selection Bracket
                        if (_selectedContact != null && _selectedContact instanceof Dictionary) {
                            var sel_cid = _selectedContact.get("id");
                            if (cid != null && sel_cid != null && cid.equals(sel_cid)) {
                                dc.setColor(Graphics.COLOR_BLUE, Graphics.COLOR_TRANSPARENT);
                                dc.setPenWidth(2);
                                dc.drawLine(tx - 10, ty - 10, tx - 5, ty - 10);
                                dc.drawLine(tx - 10, ty - 10, tx - 10, ty - 5);
                                dc.drawLine(tx + 10, ty - 10, tx + 5, ty - 10);
                                dc.drawLine(tx + 10, ty - 10, tx + 10, ty - 5);
                                dc.drawLine(tx - 10, ty + 10, tx - 5, ty + 10);
                                dc.drawLine(tx - 10, ty + 10, tx - 10, ty + 5);
        ...
        // Metrics Readout
        if (_selectedContact != null && _selectedContact instanceof Dictionary) {
            var m_name = _selectedContact.get("name") != null ? _selectedContact.get("name") : "UNKNOWN THREAT";
            ...
            var m_cog = _selectedContact.get("heading") != null ? _selectedContact.get("heading") : 0.0;
            var m_type = _selectedContact.get("type");
            var type_str = (m_type != null) ? m_type.toString() : "VESSEL";
            ...
            var s_f = 0.0; if (m_sog instanceof Float || m_sog instanceof Double) { s_f = m_sog; } else if (m_sog instanceof Number) { s_f = m_sog.toFloat(); } else if (m_sog instanceof String) { s_f = m_sog.toFloat(); }
            var c_f = 0.0; if (m_cog instanceof Float || m_cog instanceof Double) { c_f = m_cog; } else if (m_cog instanceof Number) { c_f = m_cog.toFloat(); } else if (m_cog instanceof String) { c_f = m_cog.toFloat(); }

            dc.setColor(Graphics.COLOR_BLUE, Graphics.COLOR_TRANSPARENT);
            dc.drawText(w / 2, h - 80, Graphics.FONT_XTINY, type_str + ": " + m_name.toString(), Graphics.TEXT_JUSTIFY_CENTER);
            dc.drawText(w / 2, h - 55, Graphics.FONT_XTINY, "RNG: " + r_f.format("%.1f") + "nm | BRG: " + b_f.format("%.1f"), Graphics.TEXT_JUSTIFY_CENTER);
            if (s_f > 0.0) {
                 dc.drawText(w / 2, h - 30, Graphics.FONT_XTINY, "SOG: " + s_f.format("%.1f") + "kn | COG: " + c_f.format("%.0f"), Graphics.TEXT_JUSTIFY_CENTER);
            }
        } else {
            dc.setColor(Graphics.COLOR_BLUE, Graphics.COLOR_TRANSPARENT);
            dc.drawText(w / 2, h - 50, Graphics.FONT_XTINY, "AWAITING FEED", Graphics.TEXT_JUSTIFY_CENTER);
        }
    }
}
...
class OsintRadarDelegate extends WatchUi.BehaviorDelegate {
    ...
    function onNextPage() as Boolean {
        ...
        if(view != null && view[0] instanceof OsintRadarView){
            var lrView = view[0] as OsintRadarView;
            lrView.nextContact();
            return true;
        }
        return false;
    ...
    function onKey(evt as WatchUi.KeyEvent) as Boolean {
        ...
        if (view != null && view[0] instanceof OsintRadarView) {
            var lrView = view[0] as OsintRadarView;
            if (evt.getKey() == WatchUi.KEY_UP) { lrView.prevContact(); return true; }
            if (evt.getKey() == WatchUi.KEY_DOWN) { lrView.nextContact(); return true; }
        }
        return false;
    }
    ...
    function onSwipe(swipeEvent as WatchUi.SwipeEvent) as Boolean {
        ...
        if (currentViewArray != null && currentViewArray[0] instanceof OsintRadarView) {
            ...
            if (dir == WatchUi.SWIPE_UP || dir == WatchUi.SWIPE_RIGHT) {
                lrView.zoomIn();
            } else if (dir == WatchUi.SWIPE_DOWN || dir == WatchUi.SWIPE_LEFT) {
                lrView.zoomOut();
            }
        }
        return true;
