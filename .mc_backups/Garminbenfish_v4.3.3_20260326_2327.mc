...
class WindRoseView extends WatchUi.View {
    ...
    function onUpdate(dc as Graphics.Dc) as Void {
        ...
        // 0. Fetch Physical Heading (Compass)
        var heading = 0.0f;
        var sensorInfo = Sensor.getInfo();
        if (sensorInfo != null && sensorInfo.heading != null) {
            heading = Math.toDegrees(sensorInfo.heading.toFloat());
        }

        // Header
        dc.setColor(0x00FFAA, -1);
        dc.drawText(cx, 4 + dc.getFontHeight(Graphics.FONT_XTINY), Graphics.FONT_XTINY, "WIND / WAVE ROSE", Graphics.TEXT_JUSTIFY_CENTER);
        ...
        // Compass rings & Crosshair (Un-rotated for stability? No, we rotate them for alignment)
        dc.setColor(0x003300, -1); dc.setPenWidth(1);
        dc.drawCircle(cx, cy, r);
        dc.drawCircle(cx, cy, r / 2);
        
        // NSEW labels rotated by heading
        var cardinalLabels = ["N", "E", "S", "W"];
        var cardinalAngles = [0, 90, 180, 270];
        dc.setColor(0x446644, -1);
        for (var i = 0; i < 4; i++) {
            var ang = (cardinalAngles[i] - heading + 360.0f).toNumber() % 360;
            var rad = Math.toRadians(ang.toDouble());
            var dR = (i % 2 == 0) ? r - 10 : r - 12; // Adjusted distance for N/S vs E/W
            if (i == 0) { dR = r - 10; } // N
            if (i == 2) { dR = r - 18; } // S
            dc.drawText((cx + dR * Math.sin(rad)).toNumber(), (cy - dR * Math.cos(rad)).toNumber(),
                        Graphics.FONT_XTINY, cardinalLabels[i], Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER);
        }
        ...
        // 1. Wind arrow — CYAN
        var wdRaw = _data.get("wind_dir");
        if (wdRaw != null) {
            var wd = parseBrg(wdRaw.toString());
            if (wd >= 0.0f) {
                var wBlow = (wd.toNumber() + 180 - heading.toNumber() + 720) % 360;
                var wR = (_tick % 2 == 0) ? r - 12 : r - 15;
                drawArrow(dc, cx, cy, wR, wBlow.toFloat(), 0x00FFFF, 3);
                var wBlowRad = Math.toRadians(wBlow.toDouble());
                var lR = wR + 13; if (lR > r - 4) { lR = r - 4; }
                dc.setColor(0x00FFFF, -1);
                dc.drawText((cx + lR * Math.sin(wBlowRad)).toNumber(), (cy - lR * Math.cos(wBlowRad)).toNumber(),
                            Graphics.FONT_XTINY, "W:" + wspStr, Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER);
                var wFromRad = Math.toRadians((wd - heading + 360.0f).toDouble());
                dc.setColor(0x009999, -1);
                dc.drawText((cx + 28 * Math.sin(wFromRad)).toNumber(), (cy - 28 * Math.cos(wFromRad)).toNumber(),
                            Graphics.FONT_XTINY, abbr(wdRaw.toString()), Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER);
            }
        }
        // 2. Wave arrow — WHITE
        var wvdRaw = _data.get("wave_dir");
        if (wvdRaw != null) {
            var wvd = parseBrg(wvdRaw.toString());
            if (wvd >= 0.0f) {
                var wvBlow = (wvd.toNumber() + 180 - heading.toNumber() + 720) % 360;
                drawArrow(dc, cx, cy, r - 22, wvBlow.toFloat(), 0xDDDDDD, 2);
                var wvBlowRad = Math.toRadians(wvBlow.toDouble());
                var lR = r - 22 + 13; if (lR > r - 4) { lR = r - 4; }
                dc.setColor(0xDDDDDD, -1);
                dc.drawText((cx + lR * Math.sin(wvBlowRad)).toNumber(), (cy - lR * Math.cos(wvBlowRad)).toNumber(),
                            Graphics.FONT_XTINY, "Wv:" + wvhStr, Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER);
                var wvFromRad = Math.toRadians((wvd - heading + 360.0f).toDouble());
                dc.setColor(0x888888, -1);
                dc.drawText((cx + 28 * Math.sin(wvFromRad)).toNumber(), (cy - 28 * Math.cos(wvFromRad)).toNumber(),
                            Graphics.FONT_XTINY, abbr(wvdRaw.toString()), Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER);
            }
        }
        // 3. Swell arrow — ORANGE
        var sdRaw = _data.get("swell_dir");
        if (sdRaw != null) {
            var sd = parseBrg(sdRaw.toString());
            if (sd >= 0.0f) {
                var sBlow = (sd.toNumber() + 180 - heading.toNumber() + 720) % 360;
                drawArrow(dc, cx, cy, r - 32, sBlow.toFloat(), 0xFF8800, 2);
                var sBlowRad = Math.toRadians(sBlow.toDouble());
                var lR = r - 32 + 13;
                dc.setColor(0xFF8800, -1);
                dc.drawText((cx + lR * Math.sin(sBlowRad)).toNumber(), (cy - lR * Math.cos(sBlowRad)).toNumber(),
                            Graphics.FONT_XTINY, "Sw:" + swhStr, Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER);
                var sFromRad = Math.toRadians((sd - heading + 360.0f).toDouble());
                dc.setColor(0x884400, -1);
                dc.drawText((cx + 28 * Math.sin(sFromRad)).toNumber(), (cy - 28 * Math.cos(sFromRad)).toNumber(),
                            Graphics.FONT_XTINY, abbr(sdRaw.toString()), Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER);
            }
        }
        // 4. Current arrow — GREEN
        var cdRaw = _data.get("curr_dir");
        if (cdRaw != null) {
            var cd = parseBrg(cdRaw.toString());
            if (cd >= 0.0f) {
                var cBlow = (cd.toNumber() + 180 - heading.toNumber() + 720) % 360;
                drawArrow(dc, cx, cy, r / 2 - 4, cBlow.toFloat(), 0x00CC00, 2);
                var cBlowRad = Math.toRadians(cBlow.toDouble());
                var lR = r / 2 - 4 + 13;
                dc.setColor(0x00CC00, -1);
                dc.drawText((cx + lR * Math.sin(cBlowRad)).toNumber(), (cy - lR * Math.cos(cBlowRad)).toNumber(),
                            Graphics.FONT_XTINY, "C:" + cvStr, Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER);
                var cFromRad = Math.toRadians((cd - heading + 360.0f).toDouble());
                dc.setColor(0x006600, -1);
                dc.drawText((cx + 28 * Math.sin(cFromRad)).toNumber(), (cy - 28 * Math.cos(cFromRad)).toNumber(),
                            Graphics.FONT_XTINY, abbr(cdRaw.toString()), Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER);
            }
        }


        // Bottom strip — values only (arrows + tip labels show direction)
        var row1y = h - 34; var row2y = h - 18;
        dc.setColor(0x00FFFF, -1); dc.drawText(cx/2,    row1y, Graphics.FONT_XTINY, "W:"  + wspStr + "kn",  Graphics.TEXT_JUSTIFY_CENTER);
        dc.setColor(0xDDDDDD, -1); dc.drawText(cx+cx/2, row1y, Graphics.FONT_XTINY, "Wv:" + wvhStr + "m",   Graphics.TEXT_JUSTIFY_CENTER);
        dc.setColor(0xFF8800, -1); dc.drawText(cx/2,    row2y, Graphics.FONT_XTINY, "Sw:" + swhStr + "m",   Graphics.TEXT_JUSTIFY_CENTER);
        dc.setColor(0x00CC00, -1); dc.drawText(cx+cx/2, row2y, Graphics.FONT_XTINY, "C:"  + cvStr  + "kn",  Graphics.TEXT_JUSTIFY_CENTER);
    }
}
