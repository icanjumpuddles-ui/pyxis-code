...
def fetch_wind_map(lat, lon, zoom, width, height):
    ...
    # --- 2. Load CMEMS wind grid or fallback to Open-Meteo ---
    step = 0.8
    grid_pts = [(lat + dy * step, lon + dx * step) for dy in [1, 0, -1] for dx in [-1, 0, 1]]
    wind_speeds = [None] * 9
    wind_dirs   = [None] * 9
    ...
    try:
        ...
        if os.path.exists(cache_path):
            ...
            wd = vw.get('wind_dir_deg')
            if ws is not None:
                wind_speeds = [float(ws)] * 9
            if wd is not None:
                wind_dirs = [float(wd)] * 9
    except Exception as e:
        log(f"wind_map CMEMS read: {e}")

    # Fallback: Open-Meteo if no CMEMS data
    if all(v is None for v in wind_speeds):
        try:
            ...
            if isinstance(data, list):
                ...
                wind_speeds = [data.get("current", {}).get("wind_speed_10m")]  * 9
                wind_dirs   = [data.get("current", {}).get("wind_direction_10m")] * 9
        except Exception as e:
    ...
    for i, (pt_lat, pt_lon) in enumerate(grid_pts):
        ...
        wdir = wind_dirs[i]  if i < len(wind_dirs)   else None
        color = wind_speed_to_color(spd)
        px_lon = ((pt_lon + 180.0) / 360.0) * n
        px_lat = (1.0 - math.asinh(math.tan(math.radians(pt_lat))) / math.pi) / 2.0 * n
        cx_px = int((px_lon - ctx + 1) * 256)
        cy_px = int((px_lat - cty + 1) * 256)
        radius = 40
        draw_ov.ellipse((cx_px - radius, cy_px - radius, cx_px + radius, cy_px + radius),
                        fill=color, outline=(255, 255, 255, 50), width=1)
        if spd is not None:
            draw_ov.text((cx_px - 14, cy_px - 6), f"{spd:.0f}kn", fill=(255, 255, 255, 220))
        # Wind barb (line in wind-from direction)
        if wdir is not None:
            barb_len = 22
            bx = cx_px + int(barb_len * math.sin(math.radians(wdir)))
            by = cy_px - int(barb_len * math.cos(math.radians(wdir)))
            draw_ov.line((cx_px, cy_px, bx, by), fill=(255, 255, 255, 200), width=2)
